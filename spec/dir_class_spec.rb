...
describe "NTFS::Dir class methods" do
  ...
  describe ".[]" do
    ...
    it "should enumerate the same file names as the standard Dir.glob - simple glob" do
      ...
      expect(VirtFS::VDir["*"]).to match_array(@ntfs.root_dir)
    end

    it "should enumerate the same file names as the standard Dir.glob - relative glob" do
      VirtFS.dir_chdir(@root)
      expect(VirtFS::VDir["*/*"]).to match_array(@ntfs.glob_dir)
    end
  end
  ...
  describe ".entries" do
    it "should raise RuntimeError when given a nonexistent directory" do
      expect do
        ...
        RuntimeError, /Can't find index: '\/nonexistent_directory'/
      )
    end

    it "should, given full path, return the same file names as the real Dir.entries" do
      expect(VirtFS::VDir.entries(@root)).to match_array(@ntfs.root_dir + ["."])
    end

    it "should, given relative path, return the same file names as the real Dir.entries" do
      expect(VirtFS::VDir.entries(".")).to match_array(@ntfs.root_dir + ["."])
    end
  end
  ...
  describe ".foreach" do
    ...
    it "should, given full path, return the same file names as the real Dir.foreach" do
      expect(VirtFS::VDir.foreach(@root).to_a).to match_array(@ntfs.root_dir + ["."])
    end

    it "should, given relative path, return the same file names as the real Dir.foreach" do
      expect(VirtFS::VDir.foreach(".").to_a).to match_array(@ntfs.root_dir + ["."])
    end
  end
  ...
  describe ".glob" do
    ...
    it "should enumerate the same file names as the standard Dir.glob - simple glob" do
      VirtFS.dir_chdir(@root)
      expect(VirtFS::VDir.glob("*")).to match_array(@ntfs.root_dir)
    end

    it "should enumerate the same file names as the standard Dir.glob - relative glob" do
      VirtFS.dir_chdir(@root)
      expect(VirtFS::VDir.glob("*/*")).to match_array(@ntfs.glob_dir)
    end
  end
